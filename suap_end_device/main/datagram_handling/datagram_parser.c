...
int parse_sensor_datagram(char *json, int* measurement, char* unit){
    ...
    if(main_request == NULL){
        ...
        pom = cJSON_GetObjectItem(main_request, "measurement");
        if (!cJSON_IsNumber(pom)){
        ...
        *measurement = pom->valueint;
        ...
        pom = cJSON_GetObjectItem(main_request, "unit");
        if (!cJSON_IsString(pom)){
...
int parse_actuator_datagram(char *json, int *old_state, int *new_state){
    ...
    pom = cJSON_GetObjectItem(main_request, "old_state");
        if (!cJSON_IsNumber(pom)){
    ...
    *old_state = pom->valuestring;
    ...
    pom = cJSON_GetObjectItem(main_request, "new_state");
        if (!cJSON_IsNumber(pom)){
           ESP_LOGE(pars, "Novo stanje nije broj");
           return -2;
        }
...
int parse_user_datagram(char *json, char *message, bool *input_required, int *user_input){
    ...
    pom = cJSON_GetObjectItem(main_request, "message");
        if (!cJSON_IsString(pom)){
           ESP_LOGE(pars, "poruka nije string");
           return -2;
        }
    *message = pom->valuestring;
    pom = cJSON_GetObjectItem(main_request, "input_required");
    if (cJSON_IsTrue(pom)) {
        *input_required = true;
    ...
    pom = cJSON_GetObjectItem(main_request, "user_input");
